...
interface PromocodeRepository : JpaRepository<Promocode, Long> {
    ...
    @Query("SELECT p FROM Promocode p WHERE p.code = :code AND p.isActive = true AND p.validFrom <= :now AND p.validUntil >= " +
            ":now AND (p.maxUses IS NULL OR p.usedCount < p.maxUses)")
    fun findValidPromocode(@Param("code") code: String,
